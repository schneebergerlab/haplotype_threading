...
def count_kmers_from_samples(samplekmersfin):
    ...
    #print(samplekmersfin, nodekmersfin, outfin)
    # Check if gzip
    ...
    with fo(samplekmersfin, 'r') as fin:
        for i, line in tqdm(enumerate(fin)):
            ...
            #print(line)
            if line[0].encode() in kmers: #added encode line because otherwuse we are comparing bytes to string?
                #print("ho ther")
                kmers[line[0].encode()] = int(line[1])

    print("Number of kmers in graph:",len(kmers))   
    ...
    print("Number of graph kmers in sample:",len(kmers))   
    for kdx, k in enumerate(kmers):
        if kdx < 10:
            print(k,kmers[k])
            #Think I need to ascii decode k
    ...
    print(nodekmercnts)
    zeroCountKmers=0 #sanity checker
    with open(nodekmersfin, 'r') as fin:
        for line in tqdm(fin):
            line = line.strip().split()
            nodeks[(int(line[0].encode()))] += 1
            #print("\nnodeks",nodeks)
            try:
                #print(nodekmercnts)
                #print(int(line[0]))
                ...
                #print("KeyError ",int(line[0]))
                #raise k
                zeroCountKmers+=1
                pass
            except Exception as e:
                if line == []:
                    #print("Expection: line == [] at line 78")
                    pass
                else:
                    raise e
    print("kmers skipped (zero counts?):",zeroCountKmers)
    for kdx, k in enumerate(nodeks):
        if kdx < 10:
            print(k,nodeks[k])
    for kdx, k in enumerate(nodekmercnts):
        if kdx < 10:
            print(k,nodekmercnts[k])
            #Think I need to ascii decode k 
